## DataStructure
- [list와 linked list가 무엇인가요?](#%EF%B8%8F-list와-linked-list가-무엇인가요)
- [데이터타입 중 list와 set의 차이](#%EF%B8%8F-데이터타입-중-list와-set의-차이)
- [스택과 큐의 차이](#%EF%B8%8F-스택과-큐의-차이)
- [heap에 대해 설명해주세요.](#%EF%B8%8F-heap에-대해-설명해주세요)

<br>
<br>

## 💡️ list와 linked list가 무엇인가요?
...
## 💡️ heap에 대해 설명해주세요.
> 힙이란 데이터에서 최대값과 최소값을 빠르게 찾기위해 고안된 완전 이진트리 형태의 자료구조입니다. 힙은 2가지(Max Heap과 Min Heap) 구조를 가질 수 있습니다. Max Heap의 각 부모노드는 자식노드들 보다 항상 크거나 최소한 같고, Min Heap의 부모노드는 자식노드들 보다 항상 작거나 최소한 같습니다. 힙을 사용하는 이유는 데이터를 큐나, 배열에 데이터를 넣고 최대값이나 최소값을 찾으려면 최악의 경우 일일히 하나씩 데이터를 검사해야 하기때문에 O(n)의 시간복잡도를 가지지만,, 힙을 사용하면 O(log n)으로 시간복잡도가 현격하게 줄어들기 때문입니다.
...
for i in data:
  hq.heappush(heap_data, i)
print(heap_data) # [13, 19, 33, 21, 40, 89, 71, 67] 👈 최소힙  
# 최소힙 활용
print(hq.nlargest(3, heap_data)) # [89, 71, 67] 👈 최소힙에서 큰 수 3개
print(hq.heappop(heap_data)) # 13 👈 최소힙에서 최소값 제거
print(heap_data) # [19, 21, 33, 67, 40, 89, 71] 👈 최소값 제거 후 heap 상태
```
...
for i in data:
  hq.heappush(heap_data, i)
# 최대힙 활용
print(hq.nlargest(3, heap_data)) # [-13, -19, -21] 👈 최대힙에서 가장 작은 수 3개
print(hq.heappop(heap_data)* -1) # 89 👈 최대값 제거
print(heap_data) # [-71, -40, -67, -19, -21, -33, -13] 👈제거 후 최대힙
```

- 힙과 이진트리의 차이점과 공통점은?
> 힙과 이진트리는 모두 이진트리 구조를 사용하는데 있어 공통점이 존재하나, 힙 구조는 자식 노드보다 부모노드가 크거나 같고, 또는 작거나 같습니다. 자식노드들 끼리는 어느 것이든 클 수 있고, 정렬되지 않습니다. 이에 반해 이진트리는 자식노드가 부모노드보다 크면 오른쪽, 자긍면 왼쪽으로 배치됩니다. 이에 이진탐색트리는 탐색을 위한 구조이고, 힙은 최소값 또는 최대값을 빠르게 찾기 위해 고안된 구조입니다.
