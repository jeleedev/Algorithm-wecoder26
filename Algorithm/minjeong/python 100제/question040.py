...
weight_limit = int(input('제한 무게: '))
people_limit = int(input('탑승 인원: '))
count_people = [int(input()) for _ in range(people_limit)]
total_weight = 0

for i in range(len(count_people)):
    total_weight += count_people[i]
    if total_weight > weight_limit:
        print(f'총 탈 수 있는 인원수: {i}')
        break
